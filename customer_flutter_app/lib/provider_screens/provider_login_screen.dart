...
class _ProviderLoginScreenState extends State<ProviderLoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      final res = await ProviderApiService.login(
        _emailController.text.trim(),
        _passwordController.text.trim(),
      );
      
      // Register device token for push notifications
      final String providerId = res['provider']?['id'] ?? ProviderApiService.providerId ?? 'mock_provider_user';
      try {
        await ApiService.registerDeviceToken(userId: providerId, deviceToken: 'mock_provider_fcm_token');
      } catch (tokenErr) {
        debugPrint('Failed to register device token: $tokenErr');
      }
      ...
      // Try to register token anyway for full endpoint usage representation
      try {
        await ApiService.registerDeviceToken(userId: ProviderApiService.providerId ?? 'mock_provider_user', deviceToken: 'mock_provider_fcm_token');
      } catch (_) {}
  ...
  Future<void> _register() async {
    setState(() { _isLoading = true; _errorMessage = null; });
    try {
      final res = await ProviderApiService.register(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              FutureBuilder<bool>(
                future: ProviderApiService.checkConnection(),
